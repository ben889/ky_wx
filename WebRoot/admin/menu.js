...
function setmenuhtml(json) {
    ...
    for (var i = 0; i < json.length; i++) {
        resulthtml += "<li><a href=\"javascript:bindsubmenu('" + json[i].id + "');\" id=\"" + json[i].id + "\">" + json[i].name + "</a></li>";
        if (i == 0)
...
function setleftmenuhtml(subjson, showul) {
    ...
    if (dtlength > 0) {
        ...
        for (var i = 0; i < dtlength; i++) {
            ...
            if (subjson[i].url != "" && subjson[i].url != undefined)
                url = subjson[i].url;
            subresulthtml += "<li><a href=\"" + url + "\" id=\"" + subjson[i].id + "\" target=\"mainframe\" onclick=\"" + clickfun + "\">" + subjson[i].name + "</a>";
            var isshow = 0;
            if (lev == 2 && i == 0) {
                isshow = 1;
