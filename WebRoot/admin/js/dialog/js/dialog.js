...
function Dialog(conent, btn, fun) {
    $(".mModal").remove(); $(".mDialog").remove(); var html = new Array();
    ...
    if (btn.length > 0) { html.push("    <footer><a class=\"one\" href=\"javascript:void(0)\">" + btn + "</a></footer>"); html.push("    <a class=\"x\" href=\"javascript:void(0)\">X</a>"); }
    html.push("</div>");
    ...
    $(".mModal a").css("height", b); $(".mDialog").css({ 'top': top, 'left': left });
    $(".one").off().on("click", function () { $(".mModal").remove(); $(".mDialog").remove(); if (fun != null) { $(fun).iCheck('uncheck'); } });
    $(".x").off().on("click", function () { $(".mModal").remove(); $(".mDialog").remove(); if (fun != null) { $(fun).iCheck('uncheck'); } });
}
...
// 显示正在加载...
function LoadDialog() {
    $("#mLoading").remove(); var html = new Array();
    html.push("<div id=\"mLoading\" ><div class=\"lbk\"></div><div class=\"lcont\">加载中</div></div>");
    $("body").append(html.join(""));
...
//移除正在加载...
function RemoveLoadDialog() { $("#mLoading").remove(); }
...
function pop_alert(title, conent, btn) {
    ...
    $(".btn_ok").off().on("click", function () { $(".pop_dialog").remove(); $(".mModal").remove();});
}
function pop_confirm(title, conent, btn, fun) {
    ...
    $("body").append(html.join(""));
    var d = window.innerWidth, b = window.innerHeight, c = $(".pop_dialog").innerWidth(), h = $(".pop_dialog").innerHeight(), top = (0.5 * (b - h)) + "px", left = 0.5 * (d - c) + "px";
    $(".mModal a").css("height", b); $(".pop_dialog").css({ 'top': top, 'left': left });
    $(".btn_ok").off().on("click", function () { $(".pop_dialog").remove(); $(".mModal").remove(); if (fun != null) { $(fun).iCheck('uncheck'); } });
    $(".btn_cancel").off().on("click", function () { $(".pop_dialog").remove(); $(".mModal").remove(); });
}