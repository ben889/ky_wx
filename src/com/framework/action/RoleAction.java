...
public class RoleAction extends BaseAction<Roles> {
	...
	public String list() {
		try {
			...
			List<Roles> roles = roleService.find(start, end, condition, params,
					orderby);
	...
	public String edit() {
		try {
			roleid = 0;
			role = new Roles();
			roleid = request.getParameter("roleid") != null ? Integer
					.parseInt(request.getParameter("roleid").trim()) : 0;
			if (roleid > 0) { // 修改
				Map<String, Object> mapRequest = (Map<String, Object>) ActionContext
						.getContext().get("request");
	...
	public String permission() {
		try {
			...
			List<Tabs> tabs = tabService.find(
					start, end, condition, params, orderby);
			mapRequest.put("tabs", tabs);
		} catch (Exception e) {
		}
		return "permission"; // 跳转到列表页面
	}
